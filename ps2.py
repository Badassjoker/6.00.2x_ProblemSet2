...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        if (int(x), int(y)) not in self.tilesCleaned:
        	self.tilesCleaned.append((int(x), int(y)))
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for m in range(num_trials):
    	...
    	for robot in robots:	
    		coverage = 0.0
    		count = 0
    		sum_tiles = room.getNumTiles()
    		while coverage < min_coverage:
    			robot.updatePositionAndClean()
    			# anim.update(room, robots)
    			cleaned_tiles = room.getNumCleanedTiles()
    			coverage = cleaned_tiles / float(sum_tiles)
    			count += 1
    		steps.append(count)
    	min_steps.append(sum(steps) / float(num_robots))
    	# anim.done()
    return sum(min_steps) / num_trials



# Uncomment this line to see how much your simulation takes on average
# print  runSimulation(1, 1.0, 10, 20, 1.0, 1, StandardRobot)
...
class RandomWalkRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        if self.room.isPositionInRoom(newpos):
        	...
        	self.direction = random.random() * 360
# print  runSimulation(1, 1.0, 10, 10, 1.0, 1, RandomWalkRobot)
